...
class ZoneParser:
    ...
    REGISTRY = {"alpaca": parse_alpaca.__func__, "chat": parse_chat.__func__}

    @classmethod
    def get(cls, fmt: str):
        if fmt not in cls.REGISTRY:
            raise ValueError(f"Unknown format '{fmt}'. Known: {list(cls.REGISTRY)}")
...
def source_thread(ds: Dict[str, Any], ready_q: "queue.Queue",
    ...
    name = ds["name"]
    fmt = ds["format"]
    parser = ZoneParser.get(fmt)
    n_read = n_kept = 0
    for rec in iter_source(ds):
        n_read += 1
        sample = parser(rec, name)
