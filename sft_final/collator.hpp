...
struct CollatedPool {
    ...
    bool baseline = false;
    // proxy/padding accounting (computed at collation, no GPU needed)
    int64_t batch_size = 0;      // P (rows)
    int64_t padded_width = 0;    // T (padded columns)
    int64_t padded_tokens = 0;   // P * T  == all tokens the GPU would process
    int64_t real_tokens = 0;     // sum of attention_mask (non-pad)
    int64_t pad_tokens = 0;      // padded_tokens - real_tokens
...
private:
    ...
    CollatedPool collate_chunked_(const CandidatePool& pool) const {
        ...
        finalize_counts_(out, pool, P, W);
        return out;
    }

    static void finalize_counts_(CollatedPool& out, const CandidatePool& pool,
                                 int64_t P, int64_t T) {
        out.baseline = pool.baseline;
        out.batch_size = P;
        out.padded_width = T;
        out.padded_tokens = P * T;
        out.real_tokens = out.attention_mask.sum().item<int64_t>();
        out.pad_tokens = out.padded_tokens - out.real_tokens;
    }
